...
///
/// Тот же сторож, что и для детерминизма ниже, и по той же причине: гейт
/// стоит в расписании (`movement::separation_runs`), а система, потерявшая
/// его, собирается и проходит все остальные тесты. Смысл гейта — waypoint'ы
/// сетки стоят в центрах навтайлов, и ходьба возвращает разведённую пару на
/// них же.
#[test]
