...
// Стена считается **в своих ячейках**: `roof.xy` у неё это номер панели и
// этаж (`meshing::WallFrame`), а не метры, поэтому ни ширины панели, ни высоты
// этажа, ни косины подъёма шейдеру знать не нужно — только дробная часть. Тут
// остаётся лишь то, что про рисование: доли ячейки под балкон (низ, верх,
// ширина — как на панельном доме) и толщина швов, тоже в долях ячейки.
...
// Поправка яркости **стены**: межэтажные швы, вертикальные швы панелей и
// балконы. `cell` — координаты стены в её собственных ячейках
// (`meshing::WallFrame`, вершинный атрибут): целая часть это номер панели и
// номер этажа, дробная — место внутри ячейки.
//
// Оттого тут нет ни метров, ни косины подъёма. Сколько метров в панели, решил
// сборщик — и уложил в стену **целое** число панелей и этажей, поэтому у края
// стены не бывает обрезанной панели, под карнизом полуэтажа, а балкон, сидящий
// внутри своей ячейки, ни в один край стены не упирается. Раньше сетка была
// глобальная, в метрах, и делала ровно это: резала балконы по краям стены.
//
...
fn wall_shade(cell: vec2<f32>, seed_raw: f32) -> f32 {
    // Знак посева — метка **глухой стены** (`WallFrame::without_balconies`):
    // фронтон над карнизом, узкий простенок, стена невысокого дома. Швы и
    // панели на ней те же, балконов нет; кому они полагаются, решает
    // `buildings::layers::balconies_fit` — шейдеру этого не вывести, он не
    // знает ни назначения дома, ни того, сколько всего этажей у этой стены.
    let blank = seed_raw < 0.0;
    let seed = select(seed_raw, -seed_raw - 1.0, blank);
    ...
    shade -= 0.055 * stripes(cell.y, 1.0, FLOOR_SEAM, px_cell.y);
    // шов панели — линия постоянного места по основанию, то есть параллельная
    // боковым рёбрам стены
    shade -= 0.035 * stripes(cell.x, 1.0, PANEL_SEAM, px_cell.x);
    ...
    let has_balcony = !blank && hash21(vec2<f32>(column, 0.0) + seed * 29.0) > 0.42;
    ...
    let close = visible(1.0, px_cell.y);
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if kind != 0u && params.intensity > 0.0 {
        ...
        if kind == WALL {
            ...
            shade = wall_shade(in.roof.xy, in.roof.w);
        } else {
