...
fn hash_fraction_is_stable() {
    // 1 · 2654435761 — веер бегства первой пешки
    assert_eq!(hash_fraction(2_654_435_761), 0.618_034);
    assert_eq!(hash_fraction(2_703_968_361), 0.629_566_7);
    // 1 · 3266489917 — её же ось разведения: множитель свой, иначе оба жребия
    // пешки были бы одним числом
    assert_eq!(hash_fraction(3_266_489_917), 0.760_539);
...
/// Тоже золотые, и по той же причине: поток посадки деревьев и дверей зданий
/// решает, как выглядит город, а выглядеть он обязан одинаково при каждом
/// запуске на той же выгрузке — и при любом seed'е мира.
#[test]
fn lcg_seeded_by_is_stable() {
    let mut origin = lcg_seeded_by(Vec2::ZERO);
    assert_eq!(origin(), 0.175_459_74);
    assert_eq!(origin(), 0.666_022_6);
    assert_eq!(origin(), 0.702_218_06);

    // координаты подмешиваются обе, и по разным половинам слова
    let mut elsewhere = lcg_seeded_by(Vec2::new(1234.5, 678.25));
    assert_eq!(elsewhere(), 0.875_882_57);

    // тот же засев — тот же поток: элемент не зависит от соседей по выгрузке
    let mut again = lcg_seeded_by(Vec2::ZERO);
    assert_eq!(again(), 0.175_459_74);
}
