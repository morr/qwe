...
//! ([`RoofKind`]), `7…11` — стена ([`WallKind`]). Фронтон — верх той же стены
//! и рамку берёт её же.
//!
...
/// Четыре — тот же порог, по которому `layers.rs` не даёт стене балконов:
/// одна граница, один смысл «панельного дома тут нет».
const LOW_RISE_STOREYS: f32 = 4.0;
