...
/// главная и та, что со стороны разгрузки), потолок — двенадцать, по числу
/// якорных арендаторов крупного ТЦ. Замера по OSM за этим нет и быть не может:
/// входы торговых центров там почти не размечают, так что числа взяты с
/// аэрофото и со спутника.
const BIG_BOX_MEAN: f32 = 2.0;
const BIG_BOX_MAX: usize = 12;
...
pub(super) fn cohort_of(
    ...
    // жилых когорт ей мал вдвое: у ТРЦ «Макси» (52 тыс. м², периметр за
    // километр) восемь дверей — это вход раз на сотню метров фасада, тогда как
    // на фотографии у торгового центра их дюжина, по входной группе на якорного
    // арендатора. Потолок поднят, а **закон шага остаётся законом**: сколько
    // их будет на самом деле, решает `length / BIG_BOX_ENTRANCE_SPACING`, так
    // что мелкая коробка в 1300 м² лишних дверей от этой ветки не получает.
    if big_box {
...
/// Число входов: **закон шага** — дверь на каждые `spacing` длины, но не выше
/// потолка когорты; на коротких домах, где закон шага даёт ноль, работает
/// среднее когорты.
///
/// Шаг приходит аргументом, а не берётся из [`ENTRANCE_SPACING`], потому что
/// он **свойство здания** (`mod.rs::DoorPitch`): у жилого дома подъезд каждые
/// 25 м, у торговой коробки входная группа каждые 55.
///
...
pub(super) fn entrance_count(
    ...
    let by_pitch = (length / spacing).floor() as usize;
