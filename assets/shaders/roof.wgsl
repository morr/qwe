...
// Шаг сетки размещения заплат (м) и доля клеток, где заплата есть.
const PATCH_CELL: f32 = 6.0;
const PATCH_SHARE: f32 = 0.22;
...
// меньшинство клеток, и внутри своей клетки она смещена и меньше её, так что
// соседние латки не смыкаются. Раньше здесь свой оттенок был у каждой клетки
// без исключения — это давало не заплаты, а шахматную доску во всю кровлю,
// выровненную по стенам дома. Тот же порог по доле клеток стоит на заплатах
// асфальта.
fn repair_patch(q: vec2<f32>, px: f32, seed: f32) -> f32 {
    let cell = floor(q / PATCH_CELL);
    if hash21(cell + seed * 17.0) > PATCH_SHARE {
        return 0.0;
    }
    let half = vec2<f32>(
...
fn roof_shade(
    ...
    if kind == BITUMEN {
        ...
        // Заплаты ремонта: свежая латка темнее выгоревшего ковра.
        // (`patch` — зарезервированное слово WGSL, отсюда `repair`.)
        let repair = repair_patch(vec2<f32>(u, v), px, seed) * visible(3.0, px);
        shade -= 0.075 * repair;
        // Застоявшаяся вода — тёмные пятна у парапета и в разжелобках. Латку
        // кладут как раз на протечку, поэтому пятно она **закрывает**, а не
        // складывается с ним: два затемнения друг на друге дают чёрный
        // прямоугольник посреди тёмной кляксы, чего на кровле не бывает.
        let pond = value_noise(p / 4.0 + 31.0) + 0.5;
        shade -= 0.10 * smoothstep(0.72, 0.86, pond) * visible(4.0, px) * (1.0 - repair);
    } else if kind == GRAVEL {
