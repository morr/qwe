...
pub enum WallMark {
    ...
    /// Двое таких: **фронтон** над карнизом (окно на треугольнике пришлось бы
    /// по скату, и от него осталась бы половина — ради этого продолжения
    /// фронтон и берёт раму стены под ним) и **заплата под дверью**
    /// (`buildings::layers::push_doors`), которой ячейка стены отдаётся входу
    /// целиком, чтобы окно не выглядывало из-за дверного полотна.
    Solid,
}
...
impl WallFrame {
    ...
    /// Общее тело обеих: строки обратной матрицы базиса `[основание, подъём]`,
    /// умноженные на число клеток по каждой оси.
    fn framed(
        a: Vec2,
        b: Vec2,
        lift: Vec2,
    ...
    /// разбора лежит в `roof.wgsl::wall_shade`; диапазоны разведены с запасом,
    /// чтобы порог между ними не зависел от точности числа.
