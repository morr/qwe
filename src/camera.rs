...
/// 4.5 — «общий план»: при окне 1920 × 1080 это 8640 × 4860 м, то есть карта
/// ([`MAP_SIZE`](crate::settings::MAP_SIZE) 7600 × 5700) влезает по ширине, но
/// уже не по высоте. «Вся карта в кадре» требованием никогда не было:
/// старое значение считалось по одной ширине окна 1280, а окно тянется
/// мышью. Упор держит другое — дальние ступени зум-LOD: на 4.5 балласт
/// последней ступени `RAIL_LODS` — 2.0 px при полу 1.8, его пунктир —
/// ровно 1 px при полу 1.0, лента трамвая — 1.16 px при полу 1.0. Поднять
/// упор (по высоте это 5700/1080 = 5.28) нельзя, не перетянув эти ступени
/// и не оплатив кадром обзор в 1.39 раза шире.
///
/// `pub(crate)`: таблицы зум-LOD (`map/rail.rs`, `map/tram.rs`) обязаны покрывать
/// именно этот диапазон, и их тесты берут границы отсюда, а не копией — иначе
/// расширение зума не роняет ни один тест.
pub(crate) const MIN_ZOOM: f32 = 0.05;
pub(crate) const MAX_ZOOM: f32 = 4.5;
    ...
    fn build(&self, app: &mut App) {
        app.add_plugins(PanCameraPlugin)
            ...
            .add_systems(
                Update,
                (
                    // колесо над панелью прокручивает панель, а не зумит карту
                    // (`ui/shell.rs`): тот же закон, что и для протяжки ниже —
                    // ввод, адресованный UI, миру не принадлежит
...
fn spawn_camera(
    ...
    commands.spawn((
        ...
        // окно, — кандидатом она не была никогда, и порядок у неё к тому же
        // меньше. Маркер убирает сам перебор: если в мир когда-нибудь встанет
        // вторая ОКОННАЯ камера, панели останутся здесь, а не уедут к той, у
        // которой выше `order`
        bevy::ui::IsDefaultUiCamera,
        crate::post::camera_post_process(),
        // `PanCamera` здесь ещё и маркер «камера ПОЛЬЗОВАТЕЛЯ»: закадровый
        // снимок (`dev.rs`) на три кадра поднимает вторую `Camera2d`, и всякий
...
/// (`ui/quit.rs::close_on_esc`, `Update`), и закрытие окна крестиком
...
/// `Single<&Window>` на каждом из этих мест: кадр как тип наконец проверяется
/// тестом — прежней арифметике на месте гейта требовалась живая камера с окном.
///
...
impl Viewport {
    ...
    /// Тот же кадр с запасом в `screens` экранов в стороны — тот же `screens`,
    /// что у [`Self::of`], только поверх кадра, посчитанного один раз за кадр.
    ///
    /// Запас у каждого гейта **свой и остаётся своим**, и живёт рядом с ним
    /// своей константой: [`VIEW_MARGIN`](crate::movement::VIEW_MARGIN) у
    /// диспетчера и расталкивания, `MOVEPATH_VIEW_SCREENS` у гизмо путей,
    /// `DOORS_VIEW_SCREENS` у гизмо дверей, ничего у прогрева.
