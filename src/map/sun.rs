...
//!
//! Пишет одна система — [`apply_sun`] (вне приложения — [`apply_sun_style`],
//! которым офлайн-бенч ставит солнце без мира и расписания), и запускается она
//! дважды: разово в
...
/// зданиевые слои с тенями, кроны, машины, вагоны, ограды, промзону, дорожные
/// слои (в них запечена тень моста) и юниформ материала кровель.
...
/// Ресурс — в глобаль. Единственная система, которая её пишет.
pub fn apply_sun(sun: Res<SunOnMap>) {
    apply_sun_style(sun.0);
}

/// Солнце — в глобаль **без мира**: офлайн-инструментам
/// (`examples/bench/map_meshing`), у которых нет ни приложения, ни расписания,
/// а тени они собирают теми же билдерами, что игра.
///
/// Без этого вызова свип теней меряется при компайл-таймовом дефолте статиков —
/// численно том же [`SunStyle::default`], но нигде не заявленном: прогон молча
/// зависел бы от того, что в нём не написано.
pub fn apply_sun_style(sun: SunStyle) {
    store_sun(sun.azimuth(), sun.elevation());
}
