    ...
    let mut blocks: Vec<Block> = RoofKind::ALL
        ...
        .map(|kind| Block {
            ...
            note: match kind {
                ...
                // сюда не доходит: ни `Wall` (это не кровля), ни гаражные
                // ленты (их выбирает геометрия прогона, а не назначение) в
                // `ALL` не входят, а витрина перебирает именно `ALL`. Руки
                // названы явно, а не `_`, чтобы новый **материал** нельзя
                // было добавить в `ALL` без подписи — компилятор остаётся
                // чек-листом
                RoofKind::Wall | RoofKind::GarageRow | RoofKind::GarageBlock => "",
            },
