...
- **Roof material** (`map/buildings/material.rs`) — what a roof is *covered with*, and
  ...
  `GarageRow` and `GarageBlock` are outside that choice too, and outside `ALL` with it: they
  are picked by the *geometry* of a run of garages rather than by use — see **Garage rows**
  below.)
