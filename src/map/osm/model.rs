...
/// насквозь. Смешать их значило бы однажды перекрыть 429 линиями все дворы
/// частного сектора.
#[derive(Debug, Clone)]
...
pub struct MapData {
    ...
    /// Ограды участков (`barrier=fence|wall|retaining_wall|hedge`) — только
    /// рисуются, навмеш не трогают: см. [`FenceLine`].
