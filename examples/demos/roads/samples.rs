...
//!    лучей.
//! 7. **Только настоящие данные.** Пример — вырезка кеша города
//!    (`tools/osm_crop --manifest`), не правленная руками; `title` называет
//!    тип, `note` — что на нём должен показать рендер.
...
//!
//! Рядом с вырезкой лежит `NN_*.yandex.png` — то же окно на схеме Яндекса;
...
pub(crate) struct Sample {
    ...
    /// Снимок того же окна с Яндекс Карт (`NN_*.yandex.png` рядом с вырезкой),
    ...
    manifest
        ...
        .map(|sample| {
            ...
            let (address, elements) = address(city, &osm, &bounds, at)
                .map_err(|error| format!("{}: {error}", path.display()))?;
            let reference = Some(path.with_extension("yandex.png")).filter(|path| path.exists());
            Ok(Sample {
                reference,
                elements,
                title: sample.title,
