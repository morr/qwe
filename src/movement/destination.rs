...
//! по расстоянию до собственной цели, где `(вид, PawnId)` (`order::pawn_key`)
//! разрывает ничью. Ведущий ключ — расстояние, а не идентификатор: место у
//! цели должно достаться тому, кто рядом. Хвост делает порядок полным, и
//! `sort_unstable` перестаёт зависеть от обхода запроса — та же дисциплина
//! ключа, что в `apply_pathfinding_results`.
...
/// Докуда ищется свободный слот назначения, м — дефолт [`SlotSearch`].
///
...
/// живёт на дефолте.
///
/// Читает их игра не поровну. `matching` и `slack` уходят в
/// [`assign_destination_slots`], и их дефолт воспроизводит нынешнее поведение
/// точь-в-точь. `claim_at` и `regroup` в `src/` не читает НИКТО: механизмы, к
/// которым они относятся (отложенная выдача и возврат на слот), существуют
/// только на стенде — `crowd_demo::scenario::{drive_routes, regroup_to_slot}`.
/// Их значение поэтому дефолт стенда, а не игры, и на игру не влияет никак —
/// в том числе выставленное по BRP.
///
...
pub struct SlotLab {
    ...
    /// рассуждение.
    ///
    /// Отрицательное значение допустимо (его выставляют только через BRP —
    /// стенд такой аргумент отвергает), но шаг решётки всё равно не опускается
    /// ниже одного тайла: сумму считает [`slot_side_with_slack`].
    pub slack: i32,
    ...
    /// пошла обратно; 0 — не возвращается. ТОЛЬКО СТЕНД
    /// (`crowd_demo::scenario::regroup_to_slot`).
    ///
    ...
    /// из ничего.
    ///
    /// Почему в игре его нет. Игровой `regroup_onto_slots` снят коммитом
    /// c5db16a: его запрос исключал `NeedsWanderTarget`, а осевшая пешка несёт
    /// этот тег всегда (`to_idle`, `to_pathfinding_error`, `#[require]` на
    /// спавне), так что запрос был пуст в любой сборке игры. Роль обратной
    /// связи там играет сам инвариант блуждания — пешка рано или поздно
    /// выберет новую цель. У стенда этого тега нет, поэтому копия системы
    /// живёт там и ручку читает.
    pub regroup: f32,
}
...
impl Default for SlotLab {
    fn default() -> Self {
        Self {
            ...
            slack: 0,
            claim_at: 0.0,
            // дефолт СТЕНДА: в игре поле не читает никто, а на стенде метр —
            // выигравшее по замеру значение
            // (`tools/crowd_tuning_lab/REPORT.md`, таблица «+ regroup 1.0»)
...
/// пройденном пути. Дефолт — [`SlotMatching::Batch`], по замеру
/// (`tools/separation_slots_lab/REPORT.md`, 3.1).
...
/// Сторона слота с поправкой стенда ([`SlotLab::slack`]) — ЕДИНСТВЕННЫЙ способ
/// сложить эти два числа.
///
/// Пол в один тайл принадлежит сумме, а не слагаемому: `slot_side` его уже
/// держит, но `slack` прибавляется после и снизу ничем не ограничен. При
/// `slack = -1` сторона обращалась в ноль, и `slot_of` падал на
/// `div_euclid(0)`; при `slack <= -2` решётка выворачивалась наизнанку и
/// гарантия «цели соседних слотов не ближе дистанции покоя» исчезала молча.
/// `saturating_add` — на случай `slack`, выставленного через BRP в `i32::MAX`.
pub fn slot_side_with_slack(rest_distance: f32, slack: i32) -> i32 {
    slot_side(rest_distance).saturating_add(slack).max(1)
}
