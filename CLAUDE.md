...
- **Two `PreToolUse` hooks enforce the table** (`.claude/hooks/`, wired in `.claude/settings.json`): `require-skill.sh` denies an `Edit`/`Write` of a gated path — every `.rs` under `src/`, `tests/`, `examples/` needs `bevy`, plus the domain skill of its row; `guard-bash.sh` denies a mutating git command without `git`, `cargo run` of the app or a `brp` call without `live-app`, a foreground heavy cargo command (`build`/`run`/`test`/`clippy`/`check`/`bench`/`doc`/`nextest`, and `tools/check.sh`), one of those piped through `tail`/`head`/`grep`/`rg`/`less`/`more`, `--features bevy/dynamic_linking`, a cargo command whose build directory nobody seeded (see "Running cargo so progress stays visible"; exemption: the literal `# cold-build`), and a command that **writes** a source file (`.rs`, `.wgsl`, any `.md`, `Cargo.toml`) instead of Edit/Write — a `python3 - <<PY … write_text()`, a `sed -i`, a `cat > file`. That last one is what closes the hole under the other hook: `require-skill.sh` only ever sees `Edit`/`Write`, so a file rewritten from Bash skipped the skill gate entirely. Its one exemption is the mechanical sweep CLAUDE.md already allows, and it is spelled rather than guessed — put the literal `# mechanical-sweep` in the command. `record-skill.sh` (PostToolUse/Skill) is what tells them which skills the session has loaded; `worktree-drift.sh` reports, on `EnterWorktree` and on each `Skill` load, what the worktree's branch changed under `.claude/` — because skills, hooks and settings always load from the main checkout. They are the net, not the trigger — a deny costs the whole rejected call, so load ahead of the action.
...
**A second checkout builds only what it changed — but only if its `target/` was seeded.**
An empty build directory means bevy and ~400 registry crates from scratch: 7m29s, measured.
A `target/` cloned from the main checkout means qwe and the vendored polyanya only: 1m30s,
and the clone itself is copy-on-write — ~20s and zero disk. `EnterWorktree` seeds the new
worktree by itself (`.claude/hooks/seed-worktree-target.sh`); a worktree made by hand with
`git worktree add` gets no such event, so run the seed by hand:
`tools/seed-worktree-target.sh [target-dir]`. Two things to keep straight: **never share one
`CARGO_TARGET_DIR` between checkouts** — the two builds clobber each other's `libqwe`,
because the hash is workspace-relative — and **do not invent a build-directory name**. Each
checkout uses its own plain `target/`; a `CARGO_TARGET_DIR=…/target-perf` is a directory no
seed fills, which is how one session paid the 7m29s in full. `guard-bash.sh` denies a cargo
command whose build directory holds no fingerprints while the main checkout's `target/`
