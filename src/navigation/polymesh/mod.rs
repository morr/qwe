...
use crate::settings::{POLYMESH_CHUNK_TARGET_METERS, POLYMESH_FLAT_CHUNK_METERS};

/// Границы ползунка радиуса агента ([`PolymeshDebug::agent_radius`]): инфляция
/// препятствий при триангуляции. Минимум ненулевой: по мешу теперь ходят пешки
/// шириной [`HUMAN_SIZE`], а нулевой радиус даёт контур точно по нарисованной
/// геометрии, то есть путь впритирку к стене и через щели, в которые пешка не
/// проходит. Потолок — 0.6, а не «сколько влезет»: цель блуждания в четырёх
/// случаях из пяти это вершина контура дома, `find_passable_tile_near` сдвигает
/// её на один тайл (2 м), а меш раздувает тот же контур на радиус агента, — и
/// начиная примерно с 0.7 сдвинутая цель остаётся внутри инфляции, куда снап
/// `locate` уже не дотягивается. Промахи поиска по лестнице слайдера (Тула, 600
...
///
/// Отдельной константы умолчания у этой ручки нет, и она тут не нужна:
/// [`PolymeshDebug::default`] берёт радиус из самого минимума, так что ассерт
/// «умолчание внутри диапазона» был бы тавтологией.
///
/// [`HUMAN_SIZE`]: crate::settings::HUMAN_SIZE
