...
- **dev.rs** — `TakeScreenshotEvent` (BRP-triggerable) → `screenshot.png` (gitignored);
  **`OffscreenShotEvent`** → a second camera rendering into an offscreen texture, so a
  ...
  `.claude/live-app-project.md`). Three consequences for this layer. **The main camera
  ...
  than the window; do not read that as a broken vignette. **And for the three frames of a
  ...
  skipped without a word (`ui/debug/overlays.rs::render_doors` among them).
