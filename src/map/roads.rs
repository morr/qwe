...
/// Проезжая часть — асфальт, каким он выходит на снимке: тёмно-серый, темнее
/// и земли, и тротуара. Белой (osm-carto) она была, пока не появилась
/// разметка, потом стала светло-серой; на тёмной белая линия наконец светится,
/// как на фотографии, а сетка улиц читается сама по себе.
const ROAD_COLOR: Color = Color::srgb(0.365, 0.365, 0.376);
const ALLEY_COLOR: Color = Color::srgb(0.549, 0.522, 0.463);
const WALL_COLOR: Color = Color::srgb(0.639, 0.286, 0.235);

/// Тротуар — бетон между асфальтом и землёй: светлее проезжей части в
/// полтора раза, и именно эта ступень яркости читается как бордюр.
const SIDEWALK_COLOR: Color = Color::srgb(0.529, 0.522, 0.502);
...
const ROAD_CASING_COLOR: Color = Color::srgb(0.290, 0.290, 0.298);
const ALLEY_CASING_COLOR: Color = Color::srgb(0.451, 0.427, 0.376);

/// Стены Кремля поверх зданий.
const Z_WALL: f32 = Z_BUILDING + 0.1;

/// Бордюр моста — светлый бетонный парапет над тёмным настилом, общий для
/// улиц и пешеходных мостиков. Толщины (и почему их диапазоны не
/// пересекаются) — в `map::footprint`.
const BRIDGE_CURB_COLOR: Color = Color::srgb(0.635, 0.631, 0.620);
