...
/// справа (`On`/`Off`, `Round`, `Polymesh`), а не цветом. Цвет
/// (`ButtonVariant::Primary`, `super::button_variant`) остался у кнопок, которые
/// стоят не в столбце строк, а поодиночке, и говорят не своё значение, а «где я»
/// и «в каком состоянии мир»: открытая вкладка в полоске вкладок
/// (`ui/shell.rs::sync_shell`) и пауза на кнопке скорости
/// (`ui/speed.rs::update_speed_button`). Стеной одинаковых плашек таким двум не
/// встать. Тумблеры вкладки Debug сюда не относятся — они те же
/// строки-значения (`ui/knob.rs::spawn_cycle_row`), а выбор города и вовсе не
/// кнопка, а первопартийное меню (`ui/city.rs`).
