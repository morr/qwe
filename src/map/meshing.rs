...
/// стена, ни фронтон сюда больше не входят: у обоих код `RoofKind::Wall`.
...
impl WallFrame {
    ...
    /// Та же рама, но **без балконов**: швы и панели остаются, выступов не
    /// будет. Так помечены глухие стены — фронтон над карнизом, простенок в
    /// пару панелей, стена невысокого дома — всё, чему балкон не полагается
    /// (решает `buildings::layers`).
    ///
    /// Метка едет **знаком посева** (`seed` → `-seed - 1`), потому что пятого
    /// числа в атрибуте нет, а координаты обязаны остаться теми же: швы должны
    /// идти через карниз насквозь. Различать стены можно только тем, что
    /// осталось, — знаком.
    /// Повторный вызов ничего не меняет — и это не украшение: фронтон метит
    /// себя сам поверх стены, которая на глухом доме уже помечена, а второе
    /// отрицание вернуло бы знак обратно и балконы вместе с ним.
    pub fn without_balconies(self) -> Self {
        match self.seed < 0.0 {
            true => self,
            false => Self {
                seed: -self.seed - 1.0,
