...
//!
//! **Зачем поверх спрайта рисуется круг радиуса тела.** Радиус — ползунок
//! `HumanStyle::body_radius` (0.3…1.2 м), его дефолт [`HUMAN_BODY_RADIUS`] =
//! 0.9 м: разведённая пара стоит в 1.8 м при спрайте [`HUMAN_SIZE`] = 1.0 м,
//! то есть на дефолте между спрайтами почти корпус зазора. Ниже 0.5 м на
//! ползунке дистанция покоя становится меньше спрайта, и толпа рисуется
//! слипшейся, хотя расталкивание отработало. Поэтому круг настоящего радиуса:
