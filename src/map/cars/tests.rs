...
//!
//! Лежат здесь, а не в `mod.rs`, по образцу восьми из девяти сестёр по шву
//! (`fences/tests.rs`, `rail/tests.rs`, `roads/tests.rs`; инлайном тесты
//! держит один `wagons.rs`): модуль слоя читается сверху вниз как слой, а не
//! как слой с полутысячей строк проверок в хвосте.
