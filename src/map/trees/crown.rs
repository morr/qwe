...
/// Дефолт — значения, на которых нарисован город: `CrownParams::default()`
...
/// форме нет.
///
/// Не ресурс: витрина `tree_gallery` держит набор полем своего `Tuning` и
/// крутит его вживую, а игра берёт [`CrownParams::default`] и панели для этих
/// ручек не заводит. Настраиваемой в самой игре ручке полагался бы
/// `SettingsGroup` и `register_type` в `map/mod.rs` — как у [`TreeStyle`].
#[derive(Clone, Debug)]
pub struct CrownParams {
    ...
    pub spike_floor: f32,
    /// Растяжение силуэта длинной тени вдоль её оси. У watabou выводится из
    /// длины тени: `1 + 0.5·shadowLength·d` при `shadowLength = 1`, `d = 0.8`.
    pub shadow_stretch: f32,
    /// Обратный сдвиг силуэта длинной тени по той же оси — у watabou
    /// `−R·(1 − shadowLength/4)`, то есть охват от −0.75·R до 2.05·R.
    ...
    /// силуэт (`drawTree`: `h·shadowLength > 0.5` при `shadowLength = 1`).
    /// Хвои не касается: у неё тень всегда конус-веер.
...
/// Толщины линий приходят из `settings.rs` — они там и живут по правилу
/// тюнинг-констант, и на `TREE_OUTLINE_STROKE` ссылается `tree-algo.md`;
/// остальные поля стоят литералами прямо здесь. Тело `Default` — это и есть
/// таблица «как нарисован город», и константа-посредник, читаемая ровно один
/// раз строкой ниже, к ней ничего не добавила бы: до `bd7e0ab` пять теневых
/// чисел были именованными `const` в этом же файле, а не в `settings.rs`.
