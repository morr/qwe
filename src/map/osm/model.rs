...
pub struct RoadLine {
    ...
    pub lanes: Option<u8>,
    /// Проезд внутри стоянки (`service=parking_aisle`). Внутри контура
    /// стоянки не рисуется — асфальт стоянки и есть этот проезд (`map::roads`);
    /// навмеш, двери и посадка деревьев видят его как любую дорогу.
