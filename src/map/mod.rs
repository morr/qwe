            ...
            .add_systems(
                ...
                (
                    ...
                    (
                        ...
                        .chain()
                        .run_if(in_state(AppState::Playing)),
                    // солнце — по той же причине, что у машин: в дорожный меш
                    // запечена тень моста (настил, сдвинутый по `shadow_dir()`
                    // на высоту настила через `shadow_length_scale()`), и без
                    // этого условия она осталась бы от солнца, с которым грузился
                    // город, пока все остальные тени карты едут за ползунком.
