//! Порядок записи в меш экструзии: домов в слое ([`draw_order`]) и стен
//! внутри дома ([`wall_order`]). Вопрос у них один — кто кого кроет на
//! экране, — и разложены оба одной топологической сортировкой.
//!
...
/// Порядок кладки видимых стен одного дома: дальняя раньше ближней.
///
/// Тот же вопрос, что и у домов, но на порядок проще: подъём у всех стен дома
/// **один**, поэтому в системе `(u, v)` стена — полоса постоянной толщины
/// `|lift|` над своим основанием, и в общем `u` ближе та, чьё основание ниже
/// по `v`. Никаких проб по площади: сравнение точное.
///
/// Отношение «ниже на общем участке» не зацикливается, и это свойство контура,
/// а не удача: рёбра простого кольца не пересекаются, а три отрезка, попарно
/// пересекающиеся по `u`, имеют общий `u` (отрезки на прямой), где упорядочены
/// строго. Обратных рёбер тут поэтому не бывает — [`topological`] всё равно
/// готов их проглотить.
///
/// До этого стены клались в порядке обхода контура. У дома со **ступенчатым
/// фасадом** — секции сдвинуты поперёк улицы — это ровно обратный порядок:
/// соседние стены перекрываются на экране на ширину ступени, и дальняя секция
/// ложилась поверх ближней. В тульском экстракте такая пара есть у 203 домов
/// из 7524.
pub(super) fn wall_order(walls: &[(Vec2, Vec2)], lean: Lean, lift: Vec2) -> Vec<usize> {
    let along = lean.dir();
    let across = along.perp();
    // подъём параллелен `dir()`: поперёк него стена не растёт, вдоль — ровно
    // на свою длину
    let band = lift.length();
    let bases: Vec<(Vec2, Vec2)> = walls
        .iter()
        .map(|&(a, b)| {
            let to_uv = |p: Vec2| Vec2::new(across.dot(p), along.dot(p));
            let (a, b) = (to_uv(a), to_uv(b));
            if a.x <= b.x { (a, b) } else { (b, a) }
        })
        .collect();

    let mut covered: Vec<Vec<usize>> = vec![Vec::new(); walls.len()];
    for a in 0..bases.len() {
        for b in a + 1..bases.len() {
            match nearer(bases[a], bases[b], band) {
                Some(Upper::A) => covered[a].push(b),
                Some(Upper::B) => covered[b].push(a),
                None => continue,
            }
    ...
    // база — глубина середины основания: она решает стены, которые на экране
    // не встречаются, а таких у обычной коробки все
    let mut base: Vec<usize> = (0..walls.len()).collect();
    let depth = |index: usize| {
        let (a, b) = bases[index];
        (a.y + b.y) * 0.5
    };
    base.sort_by(|&a, &b| depth(b).total_cmp(&depth(a)));
    topological(&base, &covered)
}

/// Какая из двух стен ложится поверх — та, чьё основание ближе (ниже по `v`)
/// там, где полосы встречаются. `None` — на экране они не пересекаются.
fn nearer(a: (Vec2, Vec2), b: (Vec2, Vec2), band: f32) -> Option<Upper> {
    let from = a.0.x.max(b.0.x);
    let to = a.1.x.min(b.1.x);
    if to <= from {
        return None;
    }
    // проба посередине общего участка: на его краю сидит общая вершина
    // соседних рёбер, где основания равны и сравнивать нечего
    let u = (from + to) * 0.5;
    let (va, vb) = (base_at(a, u), base_at(b, u));
    // разошлись по `v` дальше подъёма: верх ближней стены не достаёт до
    // основания дальней, и полосы на экране не встречаются
    if (va - vb).abs() >= band {
        return None;
    }
    if va < vb {
...
/// `v` основания стены на линии `u`.
fn base_at(wall: (Vec2, Vec2), u: f32) -> f32 {
    let (a, b) = wall;
    let run = b.x - a.x;
    if run <= f32::EPSILON {
        return a.y.min(b.y);
    }
    a.y + (b.y - a.y) * (u - a.x) / run
}
