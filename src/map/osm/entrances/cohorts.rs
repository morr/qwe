...
/// Когорта торговой коробки: среднее — две входные группы (у гипермаркета их
/// главная и та, что со стороны разгрузки), потолок — [`BIG_BOX_MAX`]. Замера
/// по OSM за этим нет и быть не может: входы торговых центров там почти не
/// размечают, так что числа взяты с аэрофото и со спутника.
const BIG_BOX_MEAN: f32 = 2.0;
/// Потолок входных групп коробки — **страховка, а не мерка**: он не должен
/// перебивать закон шага ни на одном доме кэша, и взят он поэтому замером, а не
/// круглым числом. Столько просит сам закон у крупнейшей коробки: у ТРЦ «Макси»
/// периметр собранного кольца 1390 м, при шаге
/// [`super::BIG_BOX_ENTRANCE_SPACING`] 55 м это двадцать пять групп. Следующая
/// по величине коробка (ТЦ «Сарафан», 648 м) просит одиннадцать, так что между
/// ними потолок не срабатывает ни на ком; связывать он начал бы только с 1430 м
/// периметра (`55 × 26`), а такого дома в кэше нет.
const BIG_BOX_MAX: usize = 25;
...
pub(super) fn cohort_of(
    ...
    // Торговая коробка меряется не подъездами, а входными группами, и потолок
    // жилых когорт ей мал втрое: у ТРЦ «Макси» (64 тыс. м², периметр 1390 м)
    // восемь дверей — это вход раз на сто семьдесят метров фасада, тогда как
    // на фотографии у торгового центра их дюжина, по входной группе на якорного
    // арендатора. Сколько их будет, решает закон шага:
    // `perimeter / BIG_BOX_ENTRANCE_SPACING` просит на «Макси» двадцать пять, и
    // потолок когорты ему не мешает. Мелкая коробка в 1300 м² с периметром под
    // 170 м получает от этой ветки три двери, а не лишние.
    if big_box {
...
/// Число входов: **закон шага** — дверь на каждые `spacing` метров `reach`, но
/// не выше потолка когорты; на коротких домах, где закон шага даёт ноль,
/// работает среднее когорты.
///
/// `reach` — то, вдоль чего меряют: у жилого дома его длина, у торговой
/// коробки весь периметр (у неё глухих сторон нет, см. `mod.rs::fill_building`).
///
/// Шаг приходит аргументом, а не берётся из [`super::ENTRANCE_SPACING`],
/// потому что он **свойство здания** (`mod.rs::DoorPitch`): у жилого дома
/// подъезд каждые 25 м, у торговой коробки входная группа каждые 55.
///
...
pub(super) fn entrance_count(
    ...
    let by_pitch = (reach / spacing).floor() as usize;
