...
//! Код `0` остаётся тому, у чего фактуры нет вовсе: фронтону и оборудованию
//! кровли.
...
/// разбирается шейдером; ноль занят «без фактуры» (фронтон, оборудование
/// кровли), поэтому коды начинаются с единицы. Стена — не ноль, а свой код
/// ([`Self::Wall`]).
