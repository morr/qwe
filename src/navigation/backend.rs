...
///
/// Обратная сторона: **систему, берущую этот ресурс, обязательно гейтить
/// `in_state(AppState::Playing)`** — вне мира ресурса нет. `SimPipeline`
/// выбирает лишь ветку режима и состоянием не гейтит; на этом уже один раз
/// упал запуск (живая цепочка поиска пути работала в `Loading`).
