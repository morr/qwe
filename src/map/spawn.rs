...
/// Земля — подложка под всем, чего нет в OSM: дворы, пустыри, вытоптанная
/// трава, проезды без тега. На снимке это тёплый серо-бурый средней светлоты,
/// а не бумага: половина города именно такая, и от её тона зависит, читается
/// карта как фотография или как чертёж.
pub const GROUND_COLOR: Color = Color::srgb(0.494, 0.478, 0.443);
...
const RESIDENTIAL_COLOR: Color = Color::srgb(0.514, 0.494, 0.447);
const INDUSTRIAL_COLOR: Color = Color::srgb(0.478, 0.478, 0.470);
pub const PARK_COLOR: Color = Color::srgb(0.404, 0.463, 0.290);
/// Лес внутри парка — полог, снятый сверху: тёмная холодная зелень, заметно
/// темнее парковой подложки, под ним и растут кроны; открытая часть парка так
/// читается как поле.
pub const WOOD_COLOR: Color = Color::srgb(0.271, 0.345, 0.224);
...
const TREE_ROW_CASING_COLOR: Color = Color::srgb(0.243, 0.310, 0.200);
/// Луг — светлее и желтее парка: выгоревшая на солнце трава без деревьев
/// обязана читаться поверх парковой заливки, иначе газон сливается с лесом.
const GRASS_COLOR: Color = Color::srgb(0.475, 0.522, 0.322);
/// Песок, пляж, грунтовая площадка — самое светлое, что есть на земле.
const SAND_COLOR: Color = Color::srgb(0.714, 0.663, 0.529);
/// Вода сверху — тёмная и мутная, серо-зелёная: Упа на снимке не голубая, а
/// почти цвета асфальта, и голубая заливка была единственным местом карты,
/// которое нельзя было принять за фотографию.
const WATER_COLOR: Color = Color::srgb(0.239, 0.310, 0.322);
...
/// Мелководье: полоса вдоль берега светлее и мутнее воды — то, что делает
/// пруд прудом, а не тёмным пятном. Три метра: у Упы это шестая часть ширины,
/// у канала — треть.
const WATER_RIM: Rim = Rim {
    width: 3.0,
    edge: Color::srgb(0.325, 0.396, 0.396),
};
...
const PARK_RIM: Rim = Rim {
    width: 2.5,
    edge: Color::srgb(0.357, 0.408, 0.255),
};
const WOOD_RIM: Rim = Rim {
    width: 3.0,
    edge: Color::srgb(0.224, 0.290, 0.184),
};
const GRASS_RIM: Rim = Rim {
    width: 2.0,
    edge: Color::srgb(0.427, 0.475, 0.290),
};
const SAND_RIM: Rim = Rim {
    width: 2.0,
    edge: Color::srgb(0.655, 0.604, 0.478),
};
