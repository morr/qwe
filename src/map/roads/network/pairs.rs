...
//! читался бы ступенькой. Стык с продолжением той же половины, у которого
//! пара тоже есть, концом куска не считается — там разводка идёт насквозь.
...
impl Pairs {
    ...
    pub fn align(
        ...
        for (road, runs) in self.runs.iter().enumerate() {
            ...
            // узлы с чужими улицами — закреплены
            let pinned: Vec<f32> = dense
                .iter()
                .zip(&along)
                .filter(|(point, _)| {
                    nodes.roads_at(**point).iter().any(|&other| {
                        other != road && (street(other).is_none() || street(other) != street(road))
                    })
                })
                .map(|(_, &at)| at)
                .collect();
    ...
    /// Разделительных с асфальтом и с газоном.
...
fn samples(path: &[Vec2]) -> Vec<(f32, Vec2, Vec2)> {
