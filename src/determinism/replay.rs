...
pub fn replay_app(
    ...
    let mut app = App::new();
    // Косметика (`draw_lunge_paths`) просит `Gizmos`, а он живёт в рендере,
    // которого здесь нет. По умолчанию Bevy роняет кадр на такой
    // невалидности параметров — здесь это ожидаемое состояние, и им место в
    // предупреждении, а не в панике. Симуляции эта система не касается: она
    // только рисует. `draw_move_paths` сюда больше не попадает — её тумблер
    // висит условием запуска, и в выключенном виде она не исполняется вовсе.
    ...
    app.add_plugins((MinimalPlugins, StatesPlugin))
        ...
        // именно теми плагинами, что перечислены выше; сброс, заведённый в
        // любом из этих девяти, страж не увидит вовсе, и узнать это из
        // `loading.rs` неоткуда. Поэтому список исключений живёт здесь:
        //
        // * `SimTimePlugin` — регулятор меряет настенные часы, а это
        //   содержимое тика. Его сбросы пинятся отдельно и вручную:
        //   `sim_time::tests::a_new_run_inherits_no_clock_and_no_regulator_memory`.
        //   Единственное исключение, у которого пин есть.
        // * `LoadingPlugin` — экран загрузки, поток скачивания, прогрев.
        //   Мир здесь собран заранее; всё, что этот плагин делает по дороге в
        //   `Live`, приходится делать руками (см. шапку модуля).
        // * `MapPlugin`, `PortalPlugin`, `CityPlugin` — карта задана
        //   аргументом, портал ресурсом, города нет вовсе.
        // * `CameraPlugin`, `UiPlugin`, `DevPlugin`, `GameDiagnosticsPlugin` —
        //   ни окна, ни рендера; на симуляцию не влияют.
        // * `ViewportPlugin` — кадр камеры как ресурс, отдельный плагин от
        //   `CameraPlugin`, и `Viewport` намеренно без `Default`. Ресурса
        //   здесь нет, и это безопасно ровно потому, что под детерминизмом ни
        //   один его читатель не исполняется: диспетчер заявок — в
        //   `SimPipeline::Live`, расталкивание снято `separation_runs`,
        //   `draw_move_paths` — выключенным тумблером, а прогрев и дверные
        //   гизмо приезжают с плагинами, которых тут нет. Читатель
        //   `Res<Viewport>`, заведённый в `BothModes`, молча не пройдёт
        //   валидацию параметров — та же беда, что была у
        //   `apply_pathfinding_results` без `SimLoad` строкой ниже.
        // * `PrefsPlugin` — прогон не должен зависеть от `settings.toml`
        //   пользователя, в этом половина смысла отдельного двора.
