...
/// вихрь портала, ореолы демонов, искры душ, — а карта, вся в цветах ≤ 1,
/// остаётся резкой, какой была.
const BLOOM_THRESHOLD: f32 = 1.1;
/// Мягкость порога: колено — [`BLOOM_THRESHOLD`] × эта доля в обе стороны.
/// При 1.0 / 0.4 светилось всё ярче 0.6, то есть белая разметка и светлые
/// крыши, и карта подёргивалась дымкой. 1.1 / 0.1 начинает с 0.99: чисто белая
/// разметка (1.0) вклада не даёт, а всё, что рисует себя в HDR — вихрь, ореол
/// демона (~1.4), искра — даёт.
const BLOOM_THRESHOLD_SOFTNESS: f32 = 0.1;
