...
///
/// Владеют значением **виды**, как и [`UrgentPath`]: демон пишет тело своего
/// вида при спавне (`settings::DemonKindStats::body_scale` — Бес
/// [`BodyScale::DEMON`], Громила втрое больше человека), человеку и
/// отладочному ходоку достаётся умолчание через `#[require]` у [`Movable`].
/// Движение спрашивает компонент и ни одного вида не называет — до этого оно
/// спрашивало `Has<Human>`, то есть «всё, что не человек, — демон», и ходок
/// молча получал демонский допуск прихода.
///
/// Расталкивание читает тот же компонент (`separation/mod.rs`) — с появлением
/// видов демонов «демон = вдвое больше» перестало быть правилом; ячейка его
/// сетки считается от самого крупного тела (`settings::MAX_BODY_SCALE`).
...
impl BodyScale {
    ...
    /// Бес вдвое больше человека — то же отношение, что у спрайтов и у
    /// расталкивания (`settings::IMP.body_scale`).
