...
/// Город для районов: одно здание на всю карту с дырами-дворами. Четыре
/// двора цепочкой на юге, от четвёртого узкий проход на север через воду по
/// **единственному** мосту к пятому двору, от него на восток шестой — сердце.
/// Дворы разнесены по клеткам `DISTRICT_GRID` (400 × 411 м), так что каждый
/// становится своим районом, а полоса к мосту даёт по району на берег.
/// `tiny_city` тут не годится: её берега пусты, районов не из чего собрать.
pub struct DistrictCity {
    pub map: MapData,
    /// Центр первого двора.
    pub portal: Vec2,
    /// Центр последнего двора.
    pub heart: Vec2,
    /// Центры шести дворов по порядку от портала к сердцу.
    pub yards: [Vec2; 6],
    /// Дворик 20 × 20 м в соседней клетке, пришитый к первому двору проходом:
    /// мельче порога осколка, должен уйти к первому двору.
    pub shard: Vec2,
    /// Полоса к мосту южнее воды.
    pub south_bank: Vec2,
    /// Полоса к мосту севернее воды.
    pub north_bank: Vec2,
    /// Вода в полосе рядом с настилом — ни в каком районе.
    pub water: Vec2,
}

pub fn district_city() -> DistrictCity {
    let yards = [
        Vec2::new(200.0, 275.0),
        Vec2::new(600.0, 250.0),
        Vec2::new(1000.0, 250.0),
        Vec2::new(1400.0, 250.0),
        Vec2::new(1400.0, 1400.0),
        Vec2::new(1800.0, 1400.0),
    ];
    let holes = vec![
        rect(Vec2::new(100.0, 200.0), Vec2::new(300.0, 350.0)),
        rect(Vec2::new(500.0, 200.0), Vec2::new(700.0, 300.0)),
        rect(Vec2::new(900.0, 200.0), Vec2::new(1100.0, 300.0)),
        // четвёртый двор, полоса на север и пятый двор — одно кольцо: полоса
        // пересекает воду, и мост через неё — единственный путь на север
        vec![
            Vec2::new(1300.0, 200.0),
            Vec2::new(1500.0, 200.0),
            Vec2::new(1500.0, 300.0),
            Vec2::new(1450.0, 300.0),
            Vec2::new(1450.0, 1300.0),
            Vec2::new(1500.0, 1300.0),
            Vec2::new(1500.0, 1500.0),
            Vec2::new(1300.0, 1500.0),
            Vec2::new(1300.0, 1300.0),
            Vec2::new(1350.0, 1300.0),
            Vec2::new(1350.0, 300.0),
            Vec2::new(1300.0, 300.0),
        ],
        rect(Vec2::new(1700.0, 1300.0), Vec2::new(1900.0, 1500.0)),
        // осколок: за границей клетки x = 400, свой проход от первого двора
        rect(Vec2::new(400.0, 320.0), Vec2::new(420.0, 340.0)),
    ];

    let mut map = MapData::default();
    map.buildings
        .push(building(rect(Vec2::ZERO, MAP_SIZE), holes));
    for (from, to) in [(300.0, 500.0), (700.0, 900.0), (1100.0, 1300.0)] {
        map.roads.push(passage(
            vec![Vec2::new(from, 250.0), Vec2::new(to, 250.0)],
            6.0,
        ));
    }
    map.roads.push(passage(
        vec![Vec2::new(300.0, 330.0), Vec2::new(400.0, 330.0)],
        6.0,
    ));
    map.roads.push(passage(
        vec![Vec2::new(1500.0, 1400.0), Vec2::new(1700.0, 1400.0)],
        6.0,
    ));
    // вода поперёк всей карты, мост — только в полосе
    map.water.push(water_area(
        rect(Vec2::new(0.0, 800.0), Vec2::new(MAP_SIZE.x, 900.0)),
        vec![],
    ));
    map.roads.push(bridge(
        vec![Vec2::new(1400.0, 780.0), Vec2::new(1400.0, 920.0)],
        6.0,
    ));
    ...
    DistrictCity {
        ...
        heart: yards[5],
        yards,
        shard: Vec2::new(410.0, 330.0),
        south_bank: Vec2::new(1400.0, 700.0),
        north_bank: Vec2::new(1400.0, 1000.0),
        water: Vec2::new(1420.0, 850.0),
    }
}
