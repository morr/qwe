...
/// Жёсткий потолок запрошенной скорости — выше него `requested` не поднять ни
/// лесенкой, ни хоткеем, ни записью по BRP.
///
/// Выше 50x просить нечего: аппаратный потолок — `fps × MAX_FRAME_DELTA`
/// (30x на ровных 60 fps), так что запрошенные сотни всё равно срезаются
/// регулятором, а панель при этом честно показывает недостижимое число.
pub const MAX_SIM_SPEED: f32 = 50.0;
/// Верх лесенки для кнопки Speed: следующий клик после неё возвращает к 1x.
/// Держится заметно ниже потолка (`fps × MAX_FRAME_DELTA` — 30x при 60 fps),
/// чтобы кнопка не просила недостижимое. Хоткей `=` доходит до `MAX_SIM_SPEED`.
pub const SPEED_CYCLE_MAX: f32 = 15.0;
