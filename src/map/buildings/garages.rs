...
/// Шаг бокса, м. Это **цель, а не делитель**: сколько боксов встанет на ленту,
/// решает округление, поэтому на своей ленте бокс чуть шире или уже — зато их
/// целое число и на торцах нет огрызка.
///
...
mod tests {
    ...
    fn a_run_holds_a_whole_number_of_bays() {
        ...
        assert!(run.bay > 1.95 + 1.0, "фургон должен войти: {}", run.bay);
    }
