...
use crate::navigation::{ArcNavmesh, Navmesh, PolymeshDebug};
...
///
/// Бэкенд — сеточный плоский A*, хотя в игре по умолчанию полигональный: в
/// повторе нет прогрева, который дожидается постройки меша, а меш, доехавший
/// посреди прогона, менял бы пути на полпути — то есть ровно то, что проверка
/// обязана исключить. Плоский A* готов с первого кадра.
pub fn replay_app(
    map: MapData,
    navmesh: Navmesh,
    portal: Vec2,
