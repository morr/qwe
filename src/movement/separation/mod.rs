...
pub struct SeparationLab {
    ...
    /// ([`SEPARATION_RATE`], [`relaxation_fraction`]).
    pub rate: f32,
    /// Страховка от телепорта: нижняя планка итогового клампа сдвига за
    /// прогон ([`SEPARATION_MAX_STEP`], [`clamped_step`]).
    ///
...
/// ([`SeparationExperiments::slide`]); наполняет прогон расталкивания там же, где
