...
// `settings.rs`: метры и множители, которые крутят снаружи (радиус агро,
// дальность броска, дистанция убийства, гистерезис), приходят из `settings.rs`
// строкой выше. Лимит на цель — инвариант, в терминах которого написан
// `ChaseClaims` (`claims.rs`), а ×1.5 и ×0.7 читаются только рядом со ступенью,
// где сравниваются с текущей дистанцией. Симметрично `FLEE_STEP`/`FLEE_SPREAD` в
// `human/decide.rs`; в `settings.rs` из species-файлов уезжает то, что
// продублировано в обоих (`WANDER_MAP_MARGIN`, коммит 432eabf).
...
/// ближе текущей минимум на 30 % (×0.7 текущей дистанции) — иначе две почти
/// равноудалённые жертвы перекидывают демона каждый такт перепрокладки, а
/// каждое переключение стоит нового запроса пути.
