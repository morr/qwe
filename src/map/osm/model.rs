...
pub enum BuildingUse {
    ...
    Commercial,
    /// Здание, которое **и есть магазин**: `building=retail|supermarket|mall`
...
/// дома, м². Порог измерен по тульскому кэшу: 68 зданий с `shop=*` делятся на
/// 18 крупноформатных (от 1363 м² «ДА!» до 52 321 м² ТРЦ «Макси») и 50 мелких
/// (крупнейший — 339 м²), и провал между ними в четыре раза шире порога, так
/// что к нему не жмётся ни один настоящий дом.
...
///
/// Про теги не спрашивает по той же причине, что и [`is_fortress_tower`]: в
/// OSM крупноформатность не размечают, её видно только по пятну.
pub fn is_big_box(area: &PolyArea) -> bool {
    area.building_use == BuildingUse::Retail && ring_area(&area.outer) >= BIG_BOX_AREA_MIN
}
