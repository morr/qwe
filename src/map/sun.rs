...
//!
//! Пишет одна система — [`apply_sun`], каждый кадр в `PreUpdate`. Это **не**
//! правило навтайла (тот пишется только когда ни один поток-читатель не жив):
//! здесь запись идёт безусловно, и держится всё на порядке внутри кадра.
//! Отсюда предел, который надо знать, добавляя читателя:
//!
//! - читать солнце можно **на главном потоке, между `PreUpdate` и концом
//!   `Update`** — там сидят и сборка мира на входе в состояние
