...
impl Lcg {
    ...
    /// Число в `[0, 1)`. Кламп не украшение: деление `u32` на `2³¹−1` в `f32`
    /// округляется вверх до ровно `1.0` на 63 состояниях генератора из двух с
    /// лишним миллиардов (p ≈ 2.9·10⁻⁸), и этого хватает, чтобы [`range`]
    /// однажды попала ровно в `to`. Верхняя граница — наибольший `f32` меньше
    /// единицы; все прочие значения кламп оставляет побитно прежними, поэтому
    /// посев городов не сдвигается. Деление в `f64` тот же `1.0` даёт (на 64
    /// состояниях) и вдобавок меняет младший бит у 1.3 % остальных значений —
    /// отвергнуто.
    ///
    /// [`range`]: Self::range
    pub(crate) fn next_f32(&mut self) -> f32 {
        self.0 = ((u64::from(self.0) * 48271) % 0x7FFF_FFFF) as u32;
        (self.0 as f32 / 2_147_483_647.0).min(1.0 - f32::EPSILON / 2.0)
    }

    /// Число почти в `[from, to)`: `next_f32` не достаёт до единицы, но сумма
    /// `from + t·(to − from)` округляется, и на длинном диапазоне результат
    /// всё ещё может совпасть с `to`. Где важна именно полуоткрытость — как в
    /// длине сцепа у вагонов, — кламп ставится на месте.
