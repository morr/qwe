...
/// «Макси»: грани перебираются по близости к дороге, у здания за километр
...
/// Зазор почти вчетверо больше обычного (45 м против 12) и разгоняет их по
/// периметру — следующая дверь просто не встаёт рядом с уже поставленной.
...
fn fill_building(
    ...
    let cohort = cohort_of(area, length, building.height, big_box);
    let pitch = DoorPitch::of(big_box);
    ...
    // периметр: у ТРЦ «Макси» 585 м «длины» при 1390 м стены давали восемь
    // дверей — вход раз на сто семьдесят метров, сообщено по кадру.
    let reach = match big_box {
        true => perimeter,
        false => length,
    };
...
/// Сколько дверей забирает себе одна грань. Сверху — шаг дома
/// ([`DoorPitch`]`::along`): длинный фасад не должен собрать все двери дома в
/// кучу. Снизу — жёсткий предел по зазору (`apart` того же шага): двери
/// раскладываются равномерно с шагом `length / (take + 1)`, поэтому `take`
/// дверей помещаются только при `length / (take + 1) >= pitch.apart`. Одну
/// грань всегда берём хотя бы под одну дверь — иначе у крошечного дома их не
/// осталось бы вовсе.
///
