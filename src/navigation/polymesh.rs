...
///
/// **Сейчас больше карты — то есть один слой и иерархия выключена по
/// умолчанию**, но расходимости на чанках больше нет. Её добили в два слоя:
/// геометрию шва (см. `SEAM_QUANTUM` и `seam_points`) и воронку самой
/// polyanya — вершина-угол на цепочке коллинеарных рёбер шва гоняла узлы
/// равной стоимости по кольцу полигонов бесконечно, лечится дедупликацией
/// точных повторов в вендоренном крейте (`vendor/polyanya`, `seen_nodes`).
/// Замер на Туле, 2000 запросов, 140 чанков: 0.7% промахов, среднее 5.3 мс,
/// худший 42 мс, память ровная — против зависания на первом же запросе с
/// 5–140 ГБ до починки. Включение — `QWE_POLYMESH_CHUNK_M=400`; по умолчанию
/// остаётся плоский слой, пока у иерархии не появится измеренное
/// преимущество (сейчас среднее то же, что у плоского).
