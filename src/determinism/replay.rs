...
pub fn replay_app(
    ...
    *app.world_mut()
    ...
    // забирает себе бэкенд (`DeterministicRun`), обнуляет тики, телеметрию,
    // часы и спавнер демонов. Плагина здесь нет, поэтому объявляем сами.
    //
    // Без объявления замороженным остаётся `Backend::default()` — ПУСТАЯ
    // всюду проходимая сетка, и весь прогон ищет пути сквозь дома; рестарт
    // же объявление делает, и повтор расходился с прогоном именно на этом.
    //
