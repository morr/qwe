...
/// Два цвета Кремля — единственное, что красится **не материалом**: его кровли
/// и стены палитры [`material`] не разбирают, а берут эти константы.
///
/// Всему остальному городу цвет **стены** назначением здания не задаётся: его
/// выбирает материал облицовки из своей палитры ([`material::wall_look`]),
/// ровно как это давно устроено у кровель. Прежние восемь констант — по одной
/// на `BuildingUse` — красили половину города (`building=yes`) в один тон, и
/// квартал панелек стоял сплошной заливкой.
///
