...
//! сетка, семь блоков. **Какой формы** — плоская, двускатная и её виды (со
//! слуховым окном, полувальмовая, ломаная, односкатная, крестовая) или
//! вальмовая (`RoofShape`), и решает её не материал, а `roofs.rs`, по контуру
//! и посеву: верхняя сетка, [`shapes`], пять контуров под всеми восемью
//! формами и под выбором самой игры — девять колонок.
//!
...
//!   контур под всеми крышами. Отказ при этом не подменяется витриной: если
//!   игра на этом контуре заказанную форму не строит, в подписи стоит та
//!   форма, что легла на самом деле, — чаще всего плоская, а там, где игра
//!   подменяет сама (полувальме не хватило конька на два среза), названа она.
//!
...
//! заполнения прямоугольника, доли вальмовых и слуховых окон, вылет ската и
//! его зажим, уклон).
    ...
    let mut blocks: Vec<Block> = RoofKind::ALL
        ...
        .map(|kind| Block {
            ...
            note: match kind {
                ...
                // сюда не доходит: гаражные ленты выбирает геометрия прогона,
                // а не назначение, поэтому в `ALL` их нет, а витрина
                // перебирает именно `ALL`. Рука названа явно, а не `_`, чтобы
                // новый **материал** нельзя было добавить в `ALL` без
                // подписи — компилятор остаётся чек-листом
                RoofKind::GarageRow | RoofKind::GarageBlock => "",
            },
