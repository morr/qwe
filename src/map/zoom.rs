...
impl<T: ZoomLods> ZoomBucket<T> {
    ...
    /// у слоя рельсов дальняя ступень стоит 45 к вершин, а ближняя 663 к, и
    /// счёт «по зуму где-то внутри ступени» врал бы уже в выборе ступени.
    /// Числа — из самого бенча (Тула, `dev`), а не из строки `rail meshing:`
    /// живого приложения: логу на macOS верить нельзя (App Nap).
