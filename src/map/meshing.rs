...
/// гасит фактуру, и нулевая ось тогда ни на что не влияет. Ни стена, ни
/// фронтон сюда больше не входят: у обоих код `RoofKind::Wall`.
...
impl MeshBuilder {
    ...
    /// Рамка, по которой красится геометрия после этого вызова; `None` — кайма,
    /// оборудование: всё, чему фактуры не полагается вовсе. У стены рамка своя,
    /// а не `None`, и у фронтона — та же, что у стены под ним.
