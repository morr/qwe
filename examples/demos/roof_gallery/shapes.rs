//! Сетка форм: один и тот же контур под всеми крышами города.
//!
//! Материал кровли решает, чем крыша **покрыта**; форму ей выбирает
//! `roofs.rs` — по назначению дома, по контуру и по посеву. Формы
//! (`RoofShape`): плоская, двускатная и её виды (со слуховым окном,
//! полувальмовая, ломаная, односкатная, крестовая), вальмовая. Витрина ставит
//! их в ряд по одному контуру, и слева от ряда — колонка «как решит игра», то
//! есть то, что на этом контуре нарисует город.
//!
...
///
/// И называется он тем, что легло, а не «плоская» наугад: чаще всего это и
/// правда плоская (`roofing_of` на отказе возвращает её), но игра подменяет и
/// внутри скатной ветки — полувальма, которой не хватило конька на два среза,
/// ложится обычной двускатной, и слуховые окна, которым не хватило габарита,
/// не ложатся вовсе. `drawn` приходит от `push_house`, то есть это форма,
/// дошедшая до меша.
pub(crate) fn cell_caption(cell: &ShapeCell, drawn: RoofShape) -> String {
    ...
    let mut lines = match (cell.shape, drawn) {
        ...
        (asked, got) => format!("{}:\nотказ, легла {}", name(asked), name(got)),
    };
