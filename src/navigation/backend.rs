...
/// собран из настоящего мира, либо его нет вовсе — и это заметят: система с
/// `Res<Backend>` не пройдёт валидацию параметров, а штатный обработчик
/// ошибок Bevy на этом паникует (тесты и сцена повтора ставят себе
/// `set_error_handler(warn)` и получают предупреждение). Громко и сразу —
/// в отличие от молча неверной геометрии.
///
/// Обратная сторона: **система, берущая этот ресурс, обязана состоять в
/// множестве [`SimPipeline`](crate::determinism::SimPipeline)** — вне мира
/// ресурса нет, а множества везут гейт на мир
/// ([`in_world`](crate::loading::in_world)) вместе с веткой режима, так что
/// своего `in_state` система больше не пишет; на этом уже один раз упал
/// запуск (живая цепочка поиска пути работала в `Loading`).
...
impl Walkable<'_> {
    ...
    /// Куда переставить застрявшего. Сперва — снап меша (допуск 0.75 м): в
