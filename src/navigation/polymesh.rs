...
///
/// **Сейчас больше карты — то есть один слой и иерархия выключена.** Швы
/// починены (см. `SEAM_QUANTUM` и `seam_points`): на 140 чанках прогон
/// `examples/polymesh_bench` доезжает до конца, 98 из 100, 2.0% промахов при
/// среднем 27 мс — вместо зависания на первом же запросе с 5–140 ГБ.
/// Но один запрос из полусотни всё ещё разворачивает фронт на секунды и
/// гигабайты, а плоский слой те же 300 запросов проходит за 6.05 мс в среднем
/// при 0.3% промахов. Пока остаточное расхождение не найдено, иерархия
/// включается только вручную — `QWE_POLYMESH_CHUNK_M=400`.
