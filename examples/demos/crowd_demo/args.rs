...
pub(crate) fn apply_lab(lab: &mut SeparationLab, knobs: &[(String, f32)]) {
    for (knob, value) in knobs {
        match knob.as_str() {
            ...
            "horizon" => lab.experiments.horizon = *value,
            "anticipation" => lab.experiments.anticipation = *value,
            "margin" => lab.experiments.anticipate_margin = *value,
            "lane-bias" => lab.experiments.lane_bias = *value,
            "compress" => lab.experiments.compress = *value,
            "compress-at" => lab.experiments.compress_at = *value,
            "steer" => lab.steer = *value,
            "steer-release" => lab.steer_release = *value,
            "idle-mobility" => lab.experiments.idle_mobility = *value,
            "arrive-slack" => lab.experiments.arrive_slack = *value,
            "slide" => lab.experiments.slide = *value,
            "pass-squeeze" => lab.pass_squeeze = *value,
            "left-share" => lab.left_share = *value,
            "stuck-compress" => lab.experiments.stuck_compress = *value,
            "stuck-after" => lab.experiments.stuck_after = *value,
            "stuck-ramp" => lab.experiments.stuck_ramp = *value,
            "hard-core" => lab.experiments.hard_core = *value,
            "slide-release" => lab.experiments.slide_release = *value,
            "crowd-sidestep" => lab.experiments.crowd_sidestep = *value,
