...
// ноль (оборудование кровли) уходит с одним вершинным цветом, как раньше, а
// стена — и фронтон, верх той же стены, — идут кодом `WALL` и своей фактурой.
//
...
// нарисовано), ширина панели и балкон в долях этажа. Балкон занимает по
// высоте от 0.12 до 0.72 этажа и 62 % ширины панели — как на панельном доме.
const FLOOR: f32 = 1.05;
const PANEL: f32 = 3.2;
const BALCONY_HIGH: f32 = 0.72;
const BALCONY_LOW: f32 = 0.12;
const BALCONY_WIDE: f32 = 0.62;
// Косина подъёма — зеркало `buildings::EXTRUDE_SKEW`: метров вправо на метр
// вверх. Стена в 2.5D — параллелограмм, её верх сдвинут на `(SKEW, 1) * drawn`,
// и без этого числа «поперёк линии стены» не перевести в «вверх по стене»
// (ветка `WALL`).
...
struct VertexOutput {
    ...
    @location(1) color: vec4<f32>,
    // рамка одна на грань (у кровли — длинная ось дома, у стены — её
    // собственная), интерполировать нечего — и код материала между двумя
    // гранями интерполировать было бы просто неверно
...
fn roof_shade(
    ...
    let u = uv.x;
    let v = uv.y;
    // Стена уходит первой и своим путём — и это не стилистика, а её цена.
    // Ранний выход фрагмента смотрит на код материала, а стена перестала быть
    // нулём; без этой развилки она считала бы весь кровельный тракт — возраст,
    // выцветание и два общих `fbm3` на пиксель, — который ей не полагается ни
    // по смыслу, ни по заданию. `roof_age` — возраст **кровли** дома, у стены
    // его нет; а общая октава в 8 м гаснет только к `px ≈ 5 м/пиксель`, то
    // есть уводила бы цвет стен и на общем плане, где вся фактура уже погасла.
    if kind == WALL {
        var shade = 0.0;
        // Ось стены — она сама, поэтому `v` идёт **поперёк её линии**, а `u`
        // вдоль неё. Но расстояние от линии — ещё не высота: верх стены сдвинут
        // на `lift = (EXTRUDE_SKEW, 1) * drawn`, и поперёк своей линии стена
        // укладывается не в `drawn`, а в `drawn * |dot((SKEW, 1), across)|`.
        // Множитель зависит от разворота стены — 1 у южного фасада, 0.4 у
        // западного, — так что без поправки один и тот же дом показывал бы
        // пять этажей с юга и два с запада. Делим на него: `h` — нарисованные
        // метры высоты по стене, одни и те же при любом развороте.
        let rise = max(abs(dot(vec2<f32>(EXTRUDE_SKEW, 1.0), across)), 1e-3);
        let h = v / rise;
        // и размер пикселя в той же мере: шаг `FLOOR` по `h` — это `FLOOR *
        // rise` в мире, гаснуть сетка обязана по нему. Заодно это гасит вырез
        // на почти вырожденной стене (`rise` → 0), которую ракурс сплющил в
        // полоску.
        let px_h = px / rise;
        // межэтажный шов — линия постоянной высоты, параллельная карнизу
        shade -= 0.055 * stripes(h, FLOOR, 0.05, px_h);
        // вертикальные швы панелей: `u` идёт вдоль стены и ракурсом не сжат
        ...
        // координат — независимый бросок на каждую ячейку, то есть шахматный
        // порядок, а на панельном доме балконы стоят столбцом во всю высоту.
        let column = floor(u / PANEL);
        let inside = fract(vec2<f32>(u / PANEL, h / FLOOR));
        let has_balcony = hash21(vec2<f32>(column, 0.0) + seed * 29.0) > 0.42;
