...
GROUPS = [
    ...
    # parse/tags.rs::is_service_track — на этих путях стоят вагоны (map/wagons.rs).
    # Белый список, а не «тег есть»: `service=crossover` — съезд между главными путями.
    (
        "REF станционные пути (вагоны)",
        'way["railway"]["service"~"^(siding|yard|spur)$"]({bbox});',
    ),
    ...
    ("landuse commercial/retail", 'way["landuse"~"^(commercial|retail)$"]({bbox});relation["landuse"~"^(commercial|retail)$"]({bbox});'),
    ("REF landuse residential (рисуем)", 'way["landuse"="residential"]({bbox});relation["landuse"="residential"]({bbox});'),
    ("REF landuse industrial/garages (рисуем)", 'way["landuse"~"^(industrial|garages)$"]({bbox});relation["landuse"~"^(industrial|garages)$"]({bbox});'),
