//! Сшивка чанков: компоненты связности слоя, поиск общих рёбер и сборка
//! отдельных слоёв в один меш с графом переходов между ними.
//!
//! Фаза **после** CDT: считать компоненты можно только до сшивки (она метит
//! индексы в `vertex.polygons` номером слоя), а сшивать — только когда все слои
//! построены. Подразбиение общей кромки, которое делает сшивку возможной, живёт
//! в [`super::seams`] и работает до триангуляции.
