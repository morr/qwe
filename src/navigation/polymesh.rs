...
impl PolymeshBuild {
    ...
    /// длина против прямой 1.061. Стоит это открытой площади: на том же наборе
    /// (`polymesh_bench`, 500 запросов, радиус 0.4) среднее 8.54 мс против 5.75,
    /// худший 121 мс против 54, промахи те же 5. Платят только маршруты с
    ...
    /// поиск дешевеет всего с 8.54 до 7.88 мс (`polymesh_bench`, 500 запросов),
    /// а изломы возвращаются с 16.2% до 26.3%, то есть отбор срезает почти всю
    /// пользу ради восьми процентов цены. Прямая старт → цель — плохой предиктор
    /// того, где путь захочет срезать: он огибает кварталы и отходит от неё на
    /// сотни метров.
