...
| `district.rs`, `bastion/*`, `combat.rs`, `corruption.rs`, `souls.rs`, `outcome.rs` — the M1 siege layer | `city-siege` |
...
- **`city-siege` — before changing the siege layer of M1** (`district.rs`, `bastion/*`, `combat.rs`, `corruption.rs`, `souls.rs`, `outcome.rs`): the district model (components inside cells, the shard rule, the label raster), the `district_city` fixture and what its tests pin, bastion sites and the quota, the strike, the corruption spread and the front, souls and summoning, the outcome, the `m1_win` acceptance stand.
