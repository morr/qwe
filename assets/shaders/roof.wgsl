...
const SHED: u32 = 13u;
// Композитная кассета гипермаркета: глухая стена крупными кассетами,
// ленточное остекление высоко под парапетом и тёмный цоколь. Ячейка у неё —
// кассетный пролёт в семь метров (`layers::BIG_BOX_BAY`), а ярус — торговый
// зал целиком, а не жилой этаж.
const BIG_BOX: u32 = 14u;
...
// Гипермаркет: ленточное остекление **высоко и не везде**. На фотографии
// глухая кассетная стена, по верху которой идёт полоса витражного стекла —
// у «Магнита» она под самым фризом, у «Леруа» на четверти пролётов. Доля
// невелика нарочно: сплошная лента по всему периметру — это уже витраж
// (`SHOPFRONT`), то есть фасад ТРЦ, а не коробки у шоссе.
//
// Верх ленты не доходит до единицы: над ней остаётся кассета, на которой
// снаружи и висит фриз вывески (`layers::push_brand_band`), и упереться
// стекло в него не должно. Держится это с обеих сторон: доли тут — доли
// **ячейки**, и фриз поэтому меряется тоже в долях яруса, от верха стены вниз
// (`layers::BRAND_BAND_SHARE`). Мерь его долей всей стены — и у коробки выше
// одного яруса он лёг бы ровно на эту ленту.
const BIG_BOX_WINDOW_LOW: f32 = 0.52;
const BIG_BOX_WINDOW_HIGH: f32 = 0.70;
const BIG_BOX_WINDOW_WIDE: f32 = 0.88;
const BIG_BOX_WINDOW_SHARE: f32 = 0.34;
// Шов между кассетами: тонкая тёмная линия по краю пролёта и одна поперёк,
// по горизонтальному стыку. Кассета — крупная гладкая панель, и весь её
// рисунок этими двумя линиями и исчерпывается.
const CASSETTE_SEAM: f32 = 0.012;
const CASSETTE_COURSE: f32 = 0.25;
// Цоколь коробки выше жилого: у зала под фризом идёт тёмная лента по низу
// стены, за которой прячется разгрузка и техника.
const BIG_BOX_PLINTH: f32 = 0.22;
...
fn wall_shade(kind: u32, cell: vec2<f32>, storeys: f32, seed_raw: f32) -> Wall {
    ...
    } else if kind == BIG_BOX {
        // кассета: шов по краю пролёта и горизонтальный стык — и больше
        // ничего. Гладкая стена, на которой всё говорят полоса вывески
        // сверху и лента стекла под ней
        out.shade -= 0.040 * stripes(cell.x, 1.0, CASSETTE_SEAM, px.x);
        out.shade -= 0.028 * stripes(cell.y, CASSETTE_COURSE, CASSETTE_SEAM, px.y);
    ...
    // у торговой коробки цоколь выше: под фризом и лентой стекла идёт тёмная
    // полоса в четверть яруса, за которой разгрузка и техника
    let plinth = select(PLINTH_HIGH, BIG_BOX_PLINTH, kind == BIG_BOX);
    out.shade -= 0.09 * f32(ground) * cell_band(inside.y, -1.0, plinth, px.y)
        * visible(1.0, px.y);
    ...
    // бросок на столбец — его спрашивают склад и торговая коробка: не всякий
    // пролёт их стены застеклён
    ...
    if kind == PANEL || kind == BRICK {
        ...
        // Гипермаркет: лента витражного стекла высоко под фризом, и только на
        // части пролётов — остальное глухая кассета. Жилого окна тут нет ни
        // одного, и это ровно то, чем торговая коробка отличается от дома:
        // пока она одевалась жилыми таблицами, стодвадцатиметровая стена
        // «Магнита» несла сорок столбцов квартирных окон.
        if ribbon < BIG_BOX_WINDOW_SHARE {
            out = with_opening(
                out,
                window_of(
                    inside,
