...
pub struct RoadLine {
    ...
    pub lanes: Option<u8>,
    /// Проезд внутри стоянки (`service=parking_aisle`). Стоянка лежит поверх
    /// дорог и кроет его сама, а места под ним раскладка не выбивает — ряды
    /// стоят по своей сетке (`map::parking::ParkingLayout`); навмеш, двери и
    /// посадка деревьев видят его как любую дорогу.
