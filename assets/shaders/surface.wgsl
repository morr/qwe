...
// а не тонкая линия по ширине покрышки. Заплата — клетка в 6 м, свежий битум
// темнее старого; грязь у бордюра — полоса в 70 см.
const RUT_OFFSET: f32 = 0.85;
const RUT_SIGMA: f32 = 0.32;
const RUT_AMP: f32 = 0.075;
const PATCH_SCALE: f32 = 6.0;
// Порог хеша, выше которого клетка — свежая заплата: доля заплат 12 %.
// (В `roof.wgsl` `PATCH_SHARE_*` — сама доля, поэтому здесь не «share».)
const PATCH_THRESHOLD: f32 = 0.88;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if params.wear > 0.0 && lanes >= 2.0 {
        ...
        let fresh = smoothstep(PATCH_THRESHOLD, PATCH_THRESHOLD + 0.04, repair);
