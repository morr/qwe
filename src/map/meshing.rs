...
/// `[ось x, ось y, код материала, посев]`. Все четыре числа **одинаковы у всех
/// вершин одной грани**: у кровли ось — длинная ось дома, у стены — её
/// собственное направление (`buildings::layers::wall_frame`). Фактура считается
/// по мировой координате, повёрнутой в эту ось, а не по развёртке, поэтому
...
/// Вершина без фактуры — фронтон, кайма, оборудование кровли. Нулевой код
/// материала гасит фактуру, и нулевая ось тогда ни на что не влияет. Стена
/// сюда больше не входит: у неё свой код (`RoofKind::Wall`).
...
impl MeshBuilder {
    ...
    /// Рамка, по которой красится геометрия после этого вызова; `None` — фронтон,
    /// кайма, оборудование: всё, чему фактуры не полагается вовсе. У стены рамка
    /// своя, а не `None`.
