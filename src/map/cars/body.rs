...
//! багажник, у хэтчбека его нет вовсе, у фургона крыша начинается сразу за
//! лобовым стеклом.
//!
...
impl CarShape {
    ...
    fn profile(self) -> Profile {
        match self {
            ...
            // Калина: багажника нет, заднее стекло почти на корме
...
pub fn push_shadow(builder: &mut MeshBuilder, car: &Car, offset: Vec2, detail: CarDetail) {
    ...
    match detail {
        ...
        _ => builder.push_convex(&car.outline(&profile, offset), SHADOW_COLOR.to_linear()),
    }
}
