...
impl ParkingLayout {
    ...
    /// ряды мест о дорогах не знают, и машины вставали поперёк неё. Стоянка
    /// лежит поверх дорог и ленту кроет, так что от дороги остаётся пустая
    /// полоса асфальта. Проезд самой стоянки (`parking_aisle`) не в счёт: ряды
    /// стоят по своей сетке, её проезды его и заменяют. Мост тоже: он над
    /// стоянкой.
