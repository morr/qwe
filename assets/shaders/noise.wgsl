...
//
// Сюда переезжает только то, что зовут **несколько** шейдеров. Помощник с одним
// потребителем остаётся у него: `dash_distance` в `surface.wgsl` (разметка
// полос) и `band` в `roof.wgsl` (проезд между рядами гаражей) — второй даже
// зовёт отсюда `visible`, и это не повод его переносить.
//
// Третий потребитель — полог кроны (`crown.wgsl`): ему нужен ровно `fbm3` — та
// же рябь и то же гашение по пикселю, что у поверхностей и кровель.
//
// Портал (`portal.wgsl`) остаётся при своих `hash21` / `value_noise` / `fbm`
// намеренно: хеш там другой, шум нецентрирован ([0, 1), а не [-0.5, 0.5]) и
// гашения по пикселю нет вовсе — вихрь считается в UV квада, а не по мировой
// позиции, и правило `visible` ему нечего держать.
